...
echo "[2/3] Building Go Bridge on VPS..."
ssh $REMOTE_USER@$VPS_IP "cd $DEPLOY_DIR/GenX_Go_Bridge && go build -o genx_bridge main.go"

echo "[3/3] Starting Services via Docker Compose..."
ssh $REMOTE_USER@$VPS_IP "cd $DEPLOY_DIR && docker compose up -d"
