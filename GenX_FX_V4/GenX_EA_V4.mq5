...
int OnInit()
{
   ...
   // WebRequest is not allowed in OnTick, so we use a timer for periodic updates
   EventSetTimer(1);
...
void OnTick()
{
   // OnTick is kept for rapid market response logic (local only)
}
...
void OnTimer()
{
   ...
   // Send data to bridge every timer event
   static int counter = 0;
   counter++;

   // Adjust frequency here (e.g., every 5 seconds)
   if(counter % 5 == 0)
   {
